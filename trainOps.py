...
class TrainOps(object):
    ...
    def load_fdf(self, type = 'source', split = 'train'):

        data = pickle.load(open(os.path.join(self.data_dir,'fdf'+type+'_' + split + '.pkl','rb')))
    ...
    def load_test_data(self, target):
        ...
        if target=='svhn':
            ...
            self.target_test_images, self.target_test_labels = self.load_fdf(type='target',split='')
    ...
    def train(self): 
        ...
        source_train_images, source_train_labels = self.load_fdf(split='train')
        source_test_images, source_test_labels = self.load_fdf(split='test')
        target_test_images, target_test_labels = self.load_test_data('fdf')
        
        print('Loaded')

        gamma_ensemble = [1,0.001,0.0001,0.00001]	
        for gamma_id,gamma in enumerate(gamma_ensemble):
            # if gamma in [1,0.001,0.0001,0.00001]:
            #     continue
            self.model = Model()
            self.load_exp_config()
            self.model_save_path = os.path.join(self.exp_dir,'model_'+str(gamma))
            if not os.path.exists(self.model_save_path):
                os.makedirs(self.model_save_path)
            self.model.gamma = gamma          
            # build a graph
            print(('Building model gamma = '+str(gamma)))
	...
	test_images, test_labels = self.load_test_data(target='fdf')
